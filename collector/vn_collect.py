...
Collects each week:
  - VN releases of the last 30 days, with cluster membership
...
Writes data/<week>/vn.json and appends to data/vn_cluster_history.csv.
...
def build_tag_index():
    ...
    Works for brand-new releases, which is the whole reason for doing it this way.
    """
    index = {}
    for cluster, tags in CLUSTERS.items():
        for tag_name, tag_id in tags.items():
            found = search_appids([VN_TAG_ID, tag_id], pages=2)
            for appid in found:
                index.setdefault(appid, set()).add(cluster)
            print(f"   {cluster} / {tag_name}: {len(found)}")
    return index
...
def main():
    ...
    print("[1/4] building cluster index from VN tag intersections ...")
    index = build_tag_index()
    print(f"   indexed {len(index)} VN appids across {len(CLUSTERS)} clusters")

    print("[2/4] finding recent VN releases ...")
    ...
    print("[3/4] sampling the visible VN market ...")
    market_ids = {}
    ...
    payload = {
        ...
        "counts": {"releases": len(releases), "market_titles": len(market),
                   ...
                   "lane_releases": len(lane_releases)},
    ...
    with open(hist, "a") as f:
        if new_file:
            f.write("week,cluster,count,share_pct,median_reviews,over_1k_reviews,"
